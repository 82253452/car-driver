...
function Buttons({price = 0, isValid, handleSubmit, id}) {
  ...
  function submit() {
    if (!droggle) {
      ...
      handleSubmit().then(d => {
        Taro.showModal({title: '确定兑换商品?'}).then(async ({confirm}) => {
          confirm && await Taro.showToast({title: '兑换成功', icon: 'none'})
          confirm && await Taro.reLaunch({url: '/pages/shopOrderList/index'})
        })
      }).catch(err => {
        console.log(err)
      })
